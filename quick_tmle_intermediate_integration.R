## ---------------------------
##
## Script name: quick_tmle_intermediate_integration
##
## Purpose of script: pipeline TMLE "rápido" con integración intermedia (PCA/autoencoders)
##
...
##
## Notes:
## - Rutas configurables para no depender de rutas absolutas.
## - Pequeños ajustes para reducir el uso de recursos.
## ---------------------------
...
### ... 1. Load data ... ###
## ---------------------- ##
num_cores <- max(1, RhpcBLASctl::get_num_cores() - 1)
RhpcBLASctl::blas_set_num_threads(num_cores)
options(mc.cores = num_cores)
set.seed(123)

data_dir <- Sys.getenv("TMLE_TCGA_DATA_DIR", unset = "data")
data_path <- file.path(data_dir, "O_Y_A_Delta_W.csv")
if (!file.exists(data_path)) {
  stop("No se encontró el archivo de datos en ", data_path, ". Ajusta TMLE_TCGA_DATA_DIR o coloca el CSV en 'data/'.")
}
...
cnv_path <- switch(
  dimen.method,
  pca = file.path(data_dir, "pca_W_cnv.csv"),
  nmf = file.path(data_dir, "nmf_W_cnv.csv"),
  file.path(data_dir, "encoder_W_cnv.csv")
)
mrna_path <- switch(
  dimen.method,
  pca = file.path(data_dir, "pca_W_mrna.csv"),
  nmf = file.path(data_dir, "nmf_W_mrna.csv"),
  file.path(data_dir, "encoder_W_mrna.csv")
)

if (!file.exists(cnv_path) || !file.exists(mrna_path)) {
  stop("Faltan archivos de reducción de dimensionalidad en ", data_dir, ".")
}

cnv.dimen <- read.csv(cnv_path)
mrna.dimen <- read.csv(mrna_path)
colnames(cnv.dimen) <- paste0("cnv_",colnames(cnv.dimen))
colnames(mrna.dimen) <- paste0("mrna_",colnames(mrna.dimen))
...
learners <- create.Learner("SL.ranger", tune = tune)
sl_rf <- mcSuperLearner(Y = Delta, X = clinical, family = binomial(), cvControl = list(V = 3),
                        SL.library = c(learners$names))
summary(sl_rf)
...
# geom_label( aes(x=0.5, y=-100, label="RAD"), color="#404080")
  ...
  SL.LinearComb.rf <- function(Y, X, family, ...) {
    ...
    sl__model <- mcSuperLearner(Y = Y, X = X, family = family$family, #cvControl = list(V = 3),
  ...
  sl_model_int <- CV.SuperLearner(Y = Y, X = W_, family = binomial(), 
...
Q1W <- SuperLearner::predict.SuperLearner(sl_model_int,newdata = data.frame(A=1, W_ %>% dplyr::select(-"A")), type="response")$pred
Q0W <- SuperLearner::predict.SuperLearner(sl_model_int,newdata = data.frame(A=0, W_ %>% dplyr::select(-"A")), type = "response")$pred
...
plot(sl_model_int) + theme_bw()

#### 
# SL.library = purrr::reduce(list(lapply(lrn_mars$names, function(x) c(x, "new.screen.randomForest_mrna")),
#                                 lapply(lrn_mars$names, function(x) c(x, "new.screen.randomForest_cnv")),
...
#                                 list(c("SL.LinearComb.SVM", "func_mrna")), 
#                                 list(c("SL.LinearComb.SVM", "func_cnv"))), append)
