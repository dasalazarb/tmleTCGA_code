...
give_O_GBM_LGG <- function(pathFeatureLabel,dimReduc,outTMLE3,index,Y_as_NaN) {
  ...
  if (identical(FALSE, file.exists(paste0(central_path, "/pathFeatureLabelKernels","/co-mod_best_results_last_run/all_TCGA_LGG_GBM.csv")))) {
    # ________________________________________________________________________________________________ #
    #### ........... Cargar archivos de GBM y LGG. Unirlos y procesarlos por varianzas. ........... ####
    # ________________________________________________________________________________________________ #
    cat("... 1. Load cnv and mrna profiles + merge these profiles (rbind) ... \n")
    ...
    g <- purrr::map(profiles, function(x) read.csv(file = paste0(pathFeatureLabel,"/co-mod_ccle_tcga_depurados/tcga_LGG_",x,"_depurado_tmle.csv")))
    g <- purrr::map(g, function(x) x[match(unique(x$X), x$X),])
    h <- purrr::map(profiles, function(x) read.csv(file = paste0(pathFeatureLabel,"/co-mod_ccle_tcga_depurados/tcga_GBM_",x,"_depurado_tmle.csv")))
    h <- purrr::map(h, function(x) x[match(unique(x$X), x$X),])
    
    giveme_dim <- function(profiles, purrrList) {
      return(purrr::pmap(list(profiles, purrrList), function(x,y) cat(paste0("Dimension of ",x," is: ", dim(y)[1], " x ", dim(y)[2], "\n")) ))
    }
    
    giveme_dim(profiles, h)
    
    jointProfiles <- function(x,y) { # funcion para fusionar gbm y lgg
      xy.col <- intersect(colnames(x), colnames(y)) ## variables en comun
      xy <- rbind(x[,xy.col], y[,xy.col]) # unir los dos perfiles
      row.names(xy) <- xy$X # dejar variable X como nombres de filas
      xy$X <- NULL ## eliminar X
      return(xy)
    }
    
    gh <- purrr::map2(g,h,jointProfiles) # unir perfiles
    gh[[2]] <- t(na.omit(t(gh[[2]]))) ## eliminar variables con NAN
    
    gh.var <- purrr::map(gh, function(x) apply(x,2,var)) # revisar varianza
    # purrr::map(h, hist)
    q.inf <- purrr::map(gh.var, function(x) quantile(x,.05,na.rm = TRUE)) ## eliminar pocos variables
    gh <- purrr::pmap(list(gh,gh.var,q.inf),function(x,y,z) x[,which(y > z)])
    # purrr::map(h, dim)
    
    gh.var <- purrr::map(gh, function(x) apply(x,2,var))
    q.sup <- purrr::map(gh.var, function(x) quantile(x,.99,na.rm = TRUE)) ## eliminar muy variables
    gh <- purrr::pmap(list(gh,gh.var,q.sup),function(x,y,z) x[,which(y < z)])
    
    for (i in 1:length(profiles)) {
      print(paste0("    * init profile ", profiles[i], " dimensions: ", dim(gh[[i]])[1], " x ", dim(gh[[i]])[2]))
    }
    ...
    # _____________________________________________________________________________________________________________________________________ #
    cat("... 2. Load gbm and lgg info and merge into glioma_info ... \n")
    lgg_info <- read.csv(paste0(pathFeatureLabel, "/co-mod_best_results_last_run/Y_A_clinical_LGG.csv"),row.names = 1); head(lgg_info); dim(lgg_info)
    gbm_info <- read.csv(paste0(pathFeatureLabel, "/co-mod_best_results_last_run/Y_A_clinical_GBM.csv"),row.names = 1); head(gbm_info); dim(gbm_info)
    ...
    # __________________________________________________________________________________________________________________ #
    cat("... 3. Column bind of cnv and mrna profiles for both projects (gbm and lgg) ... \n")
    W_ <- cbind(gh[[1]],gh[[2]])
    ...
    # _____________________________________________________________________________________________ #
    cat(" ... 4. Common patients for clinical and omic profiles for gbm and lgg ... \n")
    cat(" ... 5. Save clean data in '/co-mod_best_results_last_run/all_TCGA_LGG_GBM.csv' ... \n")
    pacientes_comunes <- purrr::reduce(list(row.names(W_), row.names(glioma_info)), intersect)
    write(pacientes_comunes,paste0(pathFeatureLabel,"/co-mod_best_results_last_run/barcodes_gbm_lgg_to_tmle.csv"))
    ...
    write.csv(all,paste0(pathFeatureLabel,"/co-mod_best_results_last_run/all_TCGA_LGG_GBM.csv"))
    write(all$bcr_patient_barcode, file = paste0(pathFeatureLabel,"/co-mod_best_results_last_run/barcodes_TCGA_LGG_GBM.csv"))
    ...
    # 
    # grid_params <- list()
    # grid_params[[1]] <- c(200) ## ntree
    # grid_params[[2]] <- c(25) ## maxnodes
    # grid_params[[3]] <- c(2000) ## mtry: number of variables randomly sampled at each split floor(sqrt(ncol(clinicalData)))
    # 
    # save.results <- data.frame(expand.grid(grid_params[[1]], grid_params[[2]], grid_params[[3]]), NRMSE=0, FPC=0)
    # contador <- 1
    # for (tree in grid_params[[1]]) {
    #   for (nodes in grid_params[[2]]) {
    #     for (variable in grid_params[[3]]) {
    #       print(paste0("tree: ", tree, ", node: ", nodes, " and variables: ", variable))
    #       clinicalData.imp <- missForest(clinicalData, parallelize="variables", mtry=variable, ntree = tree, maxnodes=nodes, verbose = TRUE)
    #       save.results[contador,4] <- clinicalData.imp$OOBerror[1]
    #       save.results[contador,5] <- clinicalData.imp$OOBerror[2]
    #       contador = contador + 1
    #     }
    #   }
    # }
    
    # save.results.1 <- save.results
    
    # for (i in 1:5) {
    #   clinicalData.imp <- missForest(clinicalData, parallelize="variables", ntree = 200, maxnodes = 25, mtry = 2000, verbose = TRUE)
    #   write.csv(x = data.frame(A=all$A, Y=all$Y, delta=all$delta, clinicalData.imp$ximp), file = paste0(pathFeatureLabel, "/co-mod_ccle_tcga_depurados/imputation_", i, ".csv"))
    # }
    
    for (i in 1:5) {
      clinicalData.imp <- read.csv(file = paste0(pathFeatureLabel, "/co-mod_ccle_tcga_depurados/imputation_", i, ".csv"))
      write.csv(x = data.frame(A=all$A, Y=all$Y, delta=all$delta, clinicalData.imp), file = paste0(pathFeatureLabel, "/co-mod_ccle_tcga_depurados/imputation_", i, ".csv"))
    }
    ...
    ## --------------------------------- ##
    cat("\tSummary of O ~ (Y,A,W_I,Delta):\n")
    cat(paste0("    *** final W.init.concat dimension: ", dim(W)[1], " x ", dim(W)[2], " \n"))
    cat(paste0("    *** final clinical dimension: ", dim(clinical)[1], " x ", dim(clinical)[2], " \n"))
    cat(paste0("    *** final A dimension: ", length(A), " - Summary (0: RAD+TMZ, 1: RAD): ", summary(as.factor(A))[[2]], " x ", summary(as.factor(A)))[[1]], " \n" )
    cat(paste0("    *** final Y dimension: ", length(Y), " - Summary: \n"))
    cat(summary(Y))
    cat(" \n")
    cat(paste0("    *** final delta dimension: ", length(delta), " - Summary (1: obsOutcome, 0: right-censored): ", summary(as.factor(delta))[[2]], " x ", summary(as.factor(delta))[[1]]) , " \n")
    
    return(list(Y, A, delta, W, clinical))
    
  } else if (dimReduc == TRUE & outTMLE3 == FALSE) {
    cat("\t * Loading W with reduction of dimensionality - NMF (func_NMF.py) - \n")
    # cat("\t 2.  is used for dimensionality reduction ... try other methods! \n")
    ...
    cat(paste0("    *** final W.init.concat dimension: ", dim(W)[1], " x ", dim(W)[2], "\n"))
    cat(paste0("    *** final clinical dimension: ", dim(clinical)[1], " x ", dim(clinical)[2], "\n"))
    cat(paste0("    *** final A dimension: ", length(A), " - Summary (1: radio+TMZ, 0: radio+TMZ+beva): ", summary(as.factor(A))[[2]], " x ", summary(as.factor(A))[[1]], "\n") )
    cat("    *** Numero de pacientes por clusters: \n")
    ...
    cat("O_data input structure for tmle3 -> O = (Y_{w/ NAs}, A, Delta, [W + clinical]) \n")
    # all.1 <- read.csv(paste0(pathFeatureLabel,"/co-mod_best_results_last_run/all_TCGA_LGG_GBM.csv"), header = TRUE, sep = ",")
    ...
    ## --------------------------------- ##
    cat("#. Final results: \n")
    cat(paste0("    *** final (W + clinical) dimension: ", dim(W)[1], " x ", dim(W)[2], " \n"))
    cat(paste0("    *** final clinical dimension: ", dim(clinical)[1], " x ", dim(clinical)[2], " \n"))
    cat(paste0("    *** final A dimension: ", length(A), " - Summary (0: RAD+TMZ, 1: RAD): ", summary(as.factor(A))[[2]], " x ", summary(as.factor(A))[[1]], " \n") )
    cat(paste0("    *** final Y dimension: ", length(Y), " - Summary:  \n"))
    print(summary(Y))
...
resumeData <- function(O) {
  ...
  cat(paste0("    *** final Y dimension: ", length(Y), " - Summary: \n"))
  print(summary(Y))
  cat(paste0("\n    *** final delta dimension: ", length(Delta), " - Summary (1: obsOutcome, 0: right-censored): ", summary(as.factor(Delta))[[2]], " x ", summary(as.factor(Delta))[[1]], "\n") )  
}
