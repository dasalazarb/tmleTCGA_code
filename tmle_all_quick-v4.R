...
### ... 1. Load data ... ###
## ---------------------- ##
if (identical(character(0), dir("G:/Mi unidad/Tutorial_2018-10/03_Resultados/DataTCGA/tmleTCGA/"))) {
  ruta <- "G:/My Drive/Tutorial_2018-10/03_Resultados/DataTCGA/tmleTCGA/"
} else {
...
# sl__model1 <- mcSuperLearner(Y = Y[A==1], X = W.new[A==1,], family = binomial(), 
#                              method = "method.NNloglik",
#                              cvControl = list(V = 3, stratifyCV = TRUE),
sl__model1 <- CV.SuperLearner(Y = Y[A==1], X = W.new[A==1,], family = binomial(), control = list(saveFitLibrary = TRUE),
                              # For a real analysis we would use V = 10.
                              cvControl = list(V = 5, stratifyCV = TRUE), innerCvControl = list(list(V=3)),
                              parallel = "multicore", verbose = TRUE,
                              SL.library = purrr::reduce(list(
                                # clinical info
                                # lapply(lrn_rf$names, function(x) c(x, "func_clinical")), 
                                list("SL.clinical.rf"), list("SL.clinical.bayesglm"),
                                # lapply(lrn_log$names, function(x) c(x, "func_clinical")), 
                                # # early integration
                                list("SL.early.rf"), list("SL.early.SVM"), list("SL.early.bayesglm"),
                                # # lapply(lrn_rf$names, function(x) c(x)), 
                                # # lapply(lrn_log$names, function(x) c(x)), 
                                # # # late integration
                                list("SL.Late.rf"), list("SL.Late.SVM"), list("SL.Late.bayesglm"), # list("SL.LinearComb.rf"),
                                # # # intermediate integration
                                list("SL.Int.rf.encoder"), list("SL.Int.SVM.encoder"), list("SL.Int.bayesglm.encoder"), #list("SL.Int.rf.encoder"),
                                # # # # intermediate integration
                                list("SL.Int.rf.pca"), list("SL.Int.SVM.pca"), list("SL.Int.bayesglm.pca"), #list("SL.Int.rf.pca"),
                                # # # # intermediate integration
                                list("SL.Int.rf.nmf"), list("SL.Int.SVM.nmf"), list("SL.Int.bayesglm.nmf") #list("SL.Int.rf.nmf")
                              ), append))

# saveRDS(object = sl__model1, file = paste0(ruta, "os_lgg_data_var", nvar, "/tmle_sl__model1_var", nvar, ".rds"))

## Todas las estrategias
# sl__model0 <- mcSuperLearner(Y = Y[A==0], X = W.new[A==0,], family = binomial(), 
#                              method = "method.NNloglik",
#                              cvControl = list(V = 3, stratifyCV = TRUE),
...
libs_final <- c("SL.clinical.rf_All", "SL.clinical.bayesglm_All")
...
modelname <- paste0("M1: ", which_model1, " -", " \n M0: ", which_model0 )
